...
### Fixed
- **ADDON_ACTION_FORBIDDEN 오류 수정**: WoW 12.0에서 `COMBAT_LOG_EVENT_UNFILTERED`가 restricted event로 지정되어 `RegisterEvent()` 호출 시 오류 발생. `RegisterUnitEvent("UNIT_SPELLCAST_SUCCEEDED", ...)` 방식으로 교체.
...
- **`C_Spell.GetSpellCharges` 반환 형식 처리**: WoW에서 테이블로 반환될 경우와 기존 다중반환 방식을 모두 처리하도록 수정.
...
- 유효하지 않은 스펠 ID 297850 제거 (WoW 기준 `GetSpellInfo` 반응 없음)
